if not os.loadAPI("turtleEx") then
	print("Cannot load turtleEx API")
	return
end
...
local function turnTo( x, z )
	print("turnTo(" .. x .. ", " .. z .. ") from (" .. xDir .. ", " .. zDir .. ")") 
...
local function moveTo( x, z )
	print("moveTo(" .. x .. ", " .. z .. ") from (" .. xPos .. ", " .. zPos .. ")") 
	...
	then
		...
		then
			...
			then
				if turtle.detect() 
				then
					if not turtleEx.digForward(numTries)
					then
						print("Unable to clear forward for move.")
						return false
					end
				end
...
local success = true
local maxHeight = height - 1
for yPos = 0, maxHeight
do
	print(" ")
	print("Build layer " .. yPos+1 .. " of " .. height)
...
-- move back down
for ht = 0, yPos do
	if not turtleEx.digDown() then
		print("Cannot clear down")
		return
	end
